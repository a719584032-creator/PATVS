...
class TestCasesPanel(wx.Panel):
    def __init__(self, parent):
        ...
        # Try to restore state if file exists
        self.restore_state()
    ...
    def on_close(self, event):
        self.save_state()  # Save the current state to a file
    ...
    def restore_state(self):
        ...
        try:
            ...
            pass  # If file doesn't exist, start with default state
    ...
    def up_file(self, event):
        ...
        with wx.FileDialog(self, "选择测试用例文件", wildcard="文本文件 (*.xlsx)|*.xlsx",
            ...
            if self.sql.select_filename_by_name(file_name):
                wx.MessageBox(f"文件 '{file_name}' 已经存在于数据库中。", "提示", wx.OK | wx.ICON_INFORMATION)
            else:
                self.sql.insert_case_by_filename(file_name, case_data)
    ...
    def PopulateTree(self, filename='Demo测试用例'):
        ...
        file_id = self.sql.select_fileid_by_file_name(filename)
        if file_id is None:
            file_id = 1  # 为空展示 demo 文件
        ...
        all_case = self.sql.select_case_by_fileid(file_id)
...
app = wx.App(False)
frame = wx.Frame(None, wx.ID_ANY, title="PATVS-v1.0.0", size=(1000, 700))
